...
sub get_genes { 
	...
	if ($genes_file) { 
		...
		for (my $i = 0; $i<= $#genes_lines;$i++) { 
			...
			my ($contig,$fr,$to,$st,$desc,$locus,$gene_name) = @line_sp[0..6];
...
sub collect_rfam_elements_from_gff { 
	...
	while (<RFAM>) {
		...
		my @line_sp = split(/\t/); 
#		$line_sp[5] =~ m/Alias=(.+);/;
		my @element_array = @line_sp[2,3,6];#[3,4,6];
...
sub calc_3end_avg_cov_nr { 
        my ($self) = @_;
        my $data_obj = $self->get_sequencing_data_obj;
        $self->save_sequencing_data_obj( $data_obj->calc_all_exp_avg_cov_nr );
}
